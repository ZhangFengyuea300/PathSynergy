...
# 加载modelB的数据
x = np.load('matrix_drug_cell.npy')   # 通路背景数据
...
for i_time in range(5):
    ...
    for epoch in range(epoch):
        ...
        T, S, Y = predict_gc(fused_model,device, combined_gat_loader_test,test_loader,model_a,model_b)
        ...
        # 保存每个epoch的AUC
        file_AUCs = 'result/GAT_CNN' + '_' + str(i_time) + '--AUCs--' + '.txt'
